...
const seedItems = [
  ...
  ['safety', 'social', '精选方向', '安全工程实习面经：建议重点检索 EHS、HSE、安全评价和应急管理', '社媒经验适合用于了解真实投递路径、岗位日常和面试问题，但请回到原平台核验信息。', 'https://search.bilibili.com/all?keyword=%E5%AE%89%E5%85%A8%E5%B7%A5%E7%A8%8B%20%E5%AE%9E%E4%B9%A0%20%E9%9D%A2%E7%BB%8F'],
...
].map(([major, category, source, title, summary, url]) => ({
  id: hash(`${major}-${category}-${title}`), major, category, platform: source, source, title, summary, url, status: 'seed'
}));

const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
const clean = text => decodeHtml(String(text || '')).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
...
function extractItems(html, pageUrl, source, majorId, majorConfig) {
  const items = [];
  ...
  while ((match = anchorRegex.exec(html)) !== null && items.length < 12) {
    ...
    let url = pageUrl;
    try { url = new URL(href, pageUrl).href; } catch {}
    items.push(makeItem({ majorId, source, title, url, summary: summarize(title, source.name, majorConfig.label), status: 'auto' }));
  }
  ...
  const pageSummary = clean(metaDescription?.[1]);
  if (items.length === 0 && pageTitle) {
    items.push(makeItem({
      ...
      url: pageUrl,
      summary: pageSummary || `已抓取到 ${source.name} 的公开搜索页，但页面结构限制了列表解析，请打开原页面查看最新结果。`,
      status: 'search-page'
    }));
  }
  return items;
...
function makeItem({ majorId, source, title, url, summary, status }) {
  return {
    id: hash(`${majorId}-${source.name}-${title}-${url}`),
    major: majorId,
...
async function scrape() {
  ...
  for (const [majorId, config] of Object.entries(majors)) {
    for (const query of config.queries) {
      for (const source of sources) {
        const url = source.makeUrl(query);
        try {
          const html = await fetchHtml(url);
          items.push(...extractItems(html, url, source, majorId, config));
        } catch (error) {
          errors.push({ source: source.name, major: majorId, query, message: error.message });
          items.push(makeItem({
            majorId,
            source,
  ...
  const output = {
    ...
    note: 'Automatically generated by scripts/scrape-feeds.mjs. Items keep titles, summaries, source names and original links only.',
    counts: {
      total: unique.length,
